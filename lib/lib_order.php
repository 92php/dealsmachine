...
function payment_info($pay_id)
{
   ...
   // return $GLOBALS['db']->getRow($sql);
	...
	$paymentArr = read_static_cache('payment', ADMIN_STATIC_CACHE_PATH);
	return $paymentArr[$pay_id];
}
...
{
    /* 计算订单各种费用之和的语句 */
    $total_fee = " (goods_amount  + tax + shipping_fee + insure_fee + pay_fee + free_shipping_fee ) AS total_fee ";
    ...
    if ($order)
    {
        $order['address']   = stripslashes($order['address']);
        $order['formated_goods_amount']   = price_format($order['goods_amount'], false);
        $order['formated_tax']            = price_format($order['tax'], false);
        $order['formated_shipping_fee']   = price_format(($order['shipping_fee']+$order['free_shipping_fee']), false);
        $order['shipping_fee']            = price_format($order['shipping_fee'], false);
        $order['free_shipping_fee']       = price_format($order['free_shipping_fee'], false);
...
{
    $sql = "SELECT rec_id, goods_id, goods_title, goods_sn, market_price, goods_number, " .
...
function order_weight_price($order_id)
{
    ...
    /* 获得购物车中商品的总重量 */
/*    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, g.is_free_shipping, ' .
                    'SUM(c.goods_price * c.goods_number) AS amount, ' .
                    'SUM(c.goods_number) AS number '.
                ' FROM ' . CART . ' AS c '.
                ' LEFT JOIN ' . GOODS . ' AS g ON g.goods_id = c.goods_id '.
                "WHERE c.session_id = '" . SESS_ID . "' GROUP BY is_free_shipping " ;
...
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_title, goods_sn, goods_number, " .
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.goods_title,g.gifts_id,g.goods_sn, g.is_on_sale, g.is_real, ".
...
function get_consignee($user_id)
{
    ...
    else
    {
        ...
        if ($user_id > 0)
        {
            /* 取默认地址 */
            $sql = "SELECT ua.*".
                    " FROM " . $GLOBALS['ecs']->table('user_address') . "AS ua, ".USERS.' AS u '.
...
{
    return "   {$alias}goods_amount + {$alias}tax + {$alias}shipping_fee + {$alias}free_shipping_fee" .
           " + {$alias}insure_fee  ";
}
...
//    }
    ...
    $parent = array(
        ...
        'goods_id'      => $package_id,
        'goods_sn'      => '',
        'goods_title'    => addslashes($package['package_name']),
