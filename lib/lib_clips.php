 ...
 * @return  array   $arr
 */
function get_collection_goods($user_id, $num = 10, $start = 0, $orders = '')
{
    $sql = 'SELECT g.goods_thumb,g.goods_id, g.goods_title,  g.cat_id, g.market_price, g.shop_price AS org_price,g.url_title, '.
    ...
    if($orders === 1) {
        $sql .= ' ORDER BY g.shop_price DESC';            
    } elseif($orders === 2) {
        $sql .= ' ORDER BY g.shop_price ASC';            
    } elseif(empty($orders)) {
    ...
    foreach ($res AS $row )
    {
        ...
        $goods_list[$row['goods_id']]['goods_title']    = $row['goods_title'];
        $goods_list[$row['goods_id']]['market_price']  = price_format($row['market_price']);
        $goods_list[$row['goods_id']]['shop_price']    = price_format($row['org_price']);
        $goods_list[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? price_format($promote_price) : '';
       $goods_list[$row['goods_id']]['url_title']      = get_details_link($row['goods_id'],$row['url_title']);
       $goods_list[$row['goods_id']]['goods_thumb']    = get_image_path(false, $row['goods_thumb']);
    }
...
function get_message_list($user_id, $user_name, $num, $start, $order_id = 0)
{
    ...
    {
        $sql .= " WHERE parent_id = 0 AND user_email = '" . $_SESSION['email'] . "' AND order_id=0 ORDER BY msg_time DESC limit $start, $num";
    }
    ...
    foreach ($res as $rows)
    {
        ...
        //{
            ...
            $reply = $GLOBALS['db']->selectinfo($sql);

            if ($reply)
            {
                $msg[$rows['msg_id']]['re_user_name']   = $reply['user_name'];
                $msg[$rows['msg_id']]['re_user_email']  = $reply['user_email'];
                $msg[$rows['msg_id']]['re_msg_time']    = local_date($GLOBALS['_CFG']['AM_time_format'], $reply['msg_time']);
        ...
        $msg[$rows['msg_id']]['msg_time']    = local_date($GLOBALS['_CFG']['AM_time_format'], $rows['msg_time']);
        $msg[$rows['msg_id']]['msg_type']    = $order_id ? $_SESSION['firstname'] : $GLOBALS['_LANG']['type'][$rows['msg_type']];
...
function add_message($message)
{
    ...
    if ($message['upload'])
    {
        ...
        {
           $err = sprintf($GLOBALS['_LANG']['upload_file_limit'], $upload_size_limit);
            return false;
        }
    ...
    {
        $err.=$GLOBALS['_LANG']['msg_title_empty'];
...
{
    $booking = array();
    $sql = "SELECT bg.rec_id, bg.goods_id, bg.goods_number, bg.booking_time, bg.dispose_note, g.goods_title ".
           "FROM " .$GLOBALS['ecs']->table('booking_goods')." AS bg , " .GOODS." AS g". " WHERE bg.goods_id = g.goods_id AND bg.user_id = '$user_id' ORDER BY bg.booking_time DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $booking[] = array('rec_id'       => $row['rec_id'],
                           ...
                           'url'          => build_uri('goods', array('gid'=>$row['goods_id']), $row['goods_title']));
    }
...
{
    $info = array();
    $sql  = "SELECT goods_title FROM " .GOODS. " WHERE goods_id = '$goods_id'";

    $info['goods_title']   = $GLOBALS['db']->getOne($sql);
    $info['goods_number'] = 1;
    $info['id']           = $goods_id;
...
function get_user_prompt ($user_id)
{
    ...
    $now = gmtime();
    /* 夺宝奇兵 */
    $sql = "SELECT act_id, goods_title, end_time " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $prompt[] = array(
                   'text'=>sprintf($GLOBALS['_LANG']['your_snatch'],$row['goods_title'], $row['act_id']),
        ...
        {
            $prompt[] = array(
                'text' => sprintf($GLOBALS['_LANG']['your_auction'], $row['goods_title'], $row['act_id']),
                'add_time' => $row['end_time']
            );
        }
    }
...
{
    $sql = "SELECT c.*, g.goods_title AS cmt_name, r.content AS reply_content, r.add_time AS reply_time ".
           " FROM " . COMMENT . " AS c ".
           " LEFT JOIN " . COMMENT . " AS r ".
           " ON r.parent_id = c.comment_id AND r.parent_id > 0 ".
           " LEFT JOIN " . GOODS . " AS g ".
           " ON c.comment_type=0 AND c.id_value = g.goods_id ".
           " WHERE c.user_id='$user_id' ORDER BY c.comment_id desc limit $start,$page_size ";
    $res = $GLOBALS['db']->arrQuery($sql);
    ...
    foreach ($res as $row)
    {
        $row['formated_add_time'] = local_date($GLOBALS['_CFG']['AM_time_format'], $row['add_time']);
        if ($row['reply_time'])
        {
            $row['formated_reply_time'] = local_date($GLOBALS['_CFG']['AM_time_format'], $row['reply_time']);
        }
    ...
    if ($to_article)
    {
        $sql = "SELECT article_id , title FROM " . ARTICLE . " WHERE " . db_create_in($to_article, 'article_id');
