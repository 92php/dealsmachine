...
class page
{
    ...
    /**
     * @var int $_omit_show_pages_num 缩略模式展示页数
     */
    private $_omit_show_pages_num = 5;
    ...
    function next_page($style='')
    {
        if($this->nowindex<$this->totalpage){
            return $this->_get_link($this->_get_url($this->nowindex+1),$this->next_page,$style, 'rel="nofollow"');
        }
            return $this->_get_link($this->_get_url($this->nowindex),$this->next_page,$style, 'rel="nofollow"');
    }
    ...
    {
        if($this->nowindex>1){
            return $this->_get_link($this->_get_url($this->nowindex-1),$this->pre_page,$style, 'rel="nofollow"');
        }
            return $this->_get_link($this->_get_url(1),$this->pre_page,$style, 'rel="nofollow"');
    }
    ...
    function first_page()
    {
        ...
        return $this->_get_link($this->_get_url(1),$this->first_page, '', 'rel="nofollow"');
        }
        return $this->_get_link($this->_get_url(1),$this->first_page, '', 'rel="nofollow"');
    }
    ...
    function last_page()
    {
        if(($this->nowindex==$this->totalpage) || ($this->totalpage == 0)){
            ...
            return $this->_get_link($this->_get_url($this->totalpage),$this->last_page, '', 'rel="nofollow"');
       }else{
            return $this->_get_link($this->_get_url($this->totalpage),$this->last_page, '', 'rel="nofollow"');
		}
    }
    ...
    function select($style='')
    {
	     if($this->is_ajax){
			 ...
			 $return='<select id="PB_Page_Select" atr="'.$this->url.'"  class="'.$style.'">';
       }else{
			if (strpos(realpath('./'),'eload_admin') === false && empty($_GET['x'])){
				$return='<select id="PB_Page_Select"  onchange="window.location.href=((this.options[this.selectedIndex].value*1)!=1)?(\''.$this->url.'\'+this.options[this.selectedIndex].value+\'.html\'):(\''.$this->url.'\');" class="'.$style.'">';
			}else{
				$return='<select id="PB_Page_Select"  onchange="window.location.href=\''.$this->url.'\'+this.options[this.selectedIndex].value" class="'.$style.'">';
			}
        }
		$page_num = 40;
		...
		if ( $begin > 2 ) $return.='<option value="1">1</option>';
		...
		if ( $this->totalpage > $page_num ) $return.='<option value="'.$this->totalpage.'">'.$this->totalpage.'</option>';
    ...
    function show($mode=1,$style='')
    {
        switch ($mode)
        {
            ...
            case '5': //skin2产品列表样式
               return   '<p class="listspan">'.$this->first_page().' '.$this->pre_page().' '.$this->nowbar().' '.$this->next_page().' '.$this->last_page().'</p> <p>Go to:</p><p class="gotoselect">'.$this->select('lie_sls').'</p>';
                break;
            case '8': //skin2产品列表样式无文件扩展名.html，直接带?page=
				$showpage = '<p class="listspan">'.$this->first_page().' '.$this->pre_page().' '.$this->nowbar().' '.$this->next_page().' '.$this->last_page().'</p> <p>Go to:</p><p class="gotoselect">'.$this->select('lie_sls').'</p>';
            	return $showpage;
                break;
            case '6':   //skin1的产品列表样式
			...
			case '11': //前台abc
               ...
               return  $this->__PerABCStr() . $this->first_page().' '.$this->pre_page().' '.$this->nowbar('','red').' '.$this->next_page().' '.$this->last_page().' page to '.$this->select();
                break;
...
/*----------------private function (私有方法)-----------------------------------------------------------*/
                ...
                if(stristr($_SERVER['QUERY_STRING'],$this->page_name.'=')){
				    ...
				    $linkfu = (strpos($_SERVER['REQUEST_URI'],'?')===false)?'?':'&';
					 ...
					 if(stristr($_SERVER['REQUEST_URI'],$this->page_name.'=')){
							...
							//地址存在页面参数
							$this->url=str_replace($this->page_name.'='.$this->nowindex,'',$_SERVER['REQUEST_URI']);
							//var_dump($this->url=str_replace($this->page_name.'='.$this->nowindex,'',$_SERVER['REQUEST_URI']));
							$last=$this->url[strlen($this->url)-1];
						...
						//var_dump($this->url=$_SERVER['REQUEST_URI'].$linkfu.$this->page_name.'=');
					 }
                }
            }
        }
		if (strpos(realpath('./'),'eload_admin') === false && empty($_GET['x'])){;
			if (strpos($this->url,'.htm')>0){
				$this->url = '/'.substr($this->url,1, (strpos($this->url, '.htm')-1));
				$this->url= str_replace('/'.$this->nowindex,'/',$this->url);
			}
			else 
			{
				$this->url = '/'.substr($this->url,1, (strpos($this->url, '?')-1));
     ...
     * @return string $url
     */
    function _get_url_new($pageno=1)
    {
		if (strpos(realpath('./'),'eload_admin') === false  && empty($_GET['x'])){
			if($pageno==1)
			{
				return str_replace("1.html",'',$this->url);
				//return str_replace("-page",'',$this->url.URLSUFFIX);
			}
			else
			{
				return $this->url.$pageno.URLSUFFIX;
				//return $this->url.'-'.$pageno.URLSUFFIX;
			}
			//return $this->url.'-'.$pageno.URLSUFFIX;
		}else{
			return $pageno<1?str_replace("page=0","page=1",$this->url.$pageno):$this->url.$pageno;
		}
    }
    ...
    function _get_url($pageno=1)
    {
        ...
        if (strpos(realpath('./'),'eload_admin') === false  && empty($_GET['x'])){
            if($pageno == 1){
                ...
                if($sortby || $_24h_ship || $display || $freeship || $page_size) {                
    ...
    /**
      * 获取链接地址
    */
    function _get_link($url,$text,$style='',$rel=''){
		if(substr($url,-7) == '?page=1'){
			$url = str_replace("?page=1","",$url);
		}
        $style=(empty($style))?'':'class="'.$style.'"';
        if($this->is_ajax){
            //如果是使用AJAX模式
            return '<a '.$style.' href="javascript:;" atr="'.$url.'" '.$rel.'>'.$text.'</a>';
           //return '<a '.$style.' href="javascript:'.$this->ajax_action_name.'(\''.$url.'\')">'.$text.'</a>';
        }else{
            return '<a '.$style.' href="'.$url.'" '.$rel.'>'.$text.'</a>';
        }
    }
    /**
      * 出错处理方式
    ...
    */
    ...
    private function _getHtmlModeOmit() {
        ...
        if (1 == $this->nowindex) {//第一页
            $html .= $this->_get_text('<span class="text">' . $this->pre_page . '</span>');
