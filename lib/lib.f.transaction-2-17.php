...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status,add_time,order_amount,promotion_code_youhuilv, " .
...
function get_user_links($user_id, $num = 10, $start = 0)
{
    /* 取得链接列表 */
    $arr    = array();

    $sql = "SELECT wj.*  ,(select count(distinct(ips)) from ".WJ_IP." where from_linkid = wj.id) as ip_count   FROM ".WJ_LINK." as wj  WHERE wj.user_id = '".$user_id."'  ORDER BY wj.id asc limit  $start,$num";
    //echo $sql;
    $res = $GLOBALS['db']->arrQuery($sql);
    //print_r($res);
    foreach ($res as $row)
    {
        ...
        $arr[] = array('id'       => $row['id'],
                       ...
                       'order_finish'=> $GLOBALS['db']->getOne("SELECT count(*) FROM ".ORDERINFO." WHERE   order_status > 0 and order_status < 9 and wj_linkid='".$row['id']."'"),
                       
                       'last_complete_issue'=> $GLOBALS['db']->getOne("SELECT SUM(order_amount) FROM ".ORDERINFO." WHERE month(FROM_UNIXTIME(pay_time,'%Y-%m-%d'))=month(curdate())-1  AND order_status > 0 and order_status < 9 and wj_linkid='".$row['id']."'"),
                       'this_complete_issue'=> $GLOBALS['db']->getOne("SELECT SUM(order_amount) FROM ".ORDERINFO." WHERE month(FROM_UNIXTIME(pay_time,'%Y-%m-%d'))=month(curdate())  AND order_status > 0 and order_status < 9 and wj_linkid='".$row['id']."'"),
                       'this_pending_issue'=> $GLOBALS['db']->getOne("SELECT SUM(order_amount) FROM ".ORDERINFO." WHERE order_status=0 and wj_linkid='".$row['id']."'"),
                       'img'=> $row['img'],
                       'visit_count'=> $row['visit_count'],
                       'reg_count'=> $row['reg_count'],
                       'order_count'=> $GLOBALS['db']->getOne("SELECT count(*) FROM ".ORDERINFO." WHERE wj_linkid='".$row['id']."'"),
...
function update_address($address)
{
    ...
    {
        /* 插入一条新记录 */
        $GLOBALS['db']->autoExecute(ADDR, $address, 'INSERT');
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    if($order['order_status']>2 && $order['order_status']<9){
		include(ROOT_PATH.'eload_admin/cache_files/ship_query.php');
		...
		foreach($ship_info as $k => $val){
			...
			$ship_info[$k]['ship_url'] = !empty($shipping_web[$shipping_method])?$shipping_web[$shipping_method]:'';
		}
...
function get_user_group_buy($user_id, $num = 10, $start = 0)
{
 ...
 //获取用户的可用积分
 function get_point_records($user_id, $num = 10, $start = 0)
{
     //获取用户的可用积分
    $arr    = array();

    $sql = "SELECT * FROM ".POINT_RECORD .
           " WHERE user_id = '$user_id' ORDER BY adddate asc limit  $start,$num";
    $res = $GLOBALS['db']->arrQuery($sql);
    foreach ($res as $k=>$v)
    {	        
