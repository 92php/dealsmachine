...
class cls_image
{
    ...
    function upload_image($upload, $dir = '', $img_name = '')
    {
        ...
        if (!file_exists($dir))
        {
            ...
            {
                /* 创建目录失败 */
                $this->error_msg = sprintf('目标目录属性为只读，创建失败', $dir);
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='')
    {
       ...
       // echo getimagesize("../uploads/review_upload/1307030678942818385.jpg");
        if (!$org_info)
        {
        ...
        {
            $filename .= '.jpg';
            imagejpeg($img_thumb, $dir . $filename, 97);
        }
        elseif (function_exists('imagegif'))
        {
            $filename .= '.gif';
            imagegif($img_thumb, $dir . $filename, 97);
        }
        elseif (function_exists('imagepng'))
        {
            $filename .= '.png';
            imagepng($img_thumb, $dir . $filename, 97);
        }
        else
        {
      ...
      /* 如果水印的位置为0，则返回原图 */
        ...
        switch ($watermark_place)
        {
            ...
            default:
                ...
                $y = $source_info[1]*2/3 - $watermark_info[1]*2/3;
        ...
        switch ($source_info[2] )
        {
            case 'image/gif':
            case 1:
                imagegif($source_handle,  $target, 97);
                break;

            case 'image/pjpeg':
            case 'image/jpeg':
            case 2:
                imagejpeg($source_handle, $target, 97);
                break;

            case 'image/x-png':
            case 'image/png':
            case 3:
                imagepng($source_handle,  $target, 97);
                break;
