...
$mail_subject ="Rosewholesale 发送评论邀请邮件成功：$i 封；失败：$j 封";
$mail_body    = "看标题";
...
function review_invite()
{
	global $db;
	global $host;	
    $sql = "select distinct(order_sn) as order_sn,min(add_time) a from eload_shipping_details  group by order_sn having a between (UNIX_TIMESTAMP(curdate())-31*24*3600) and (UNIX_TIMESTAMP(curdate())-30*24*3600)";
	...
	foreach ($rev as $k=>$v){   
		$review[$v['goods_id']]=$v;
		$review[$v['goods_id']]['avg_rate_img'] = $host.showRateS($v['avg_rate']);		
	}
	foreach ($arr as $k=>$v){
