   ...
   //echo 1233;
    ...
    if ($result === false)
    {
      ...
      //  if ($_REQUEST['is_ajax'] == 1)
       ...
       // }
        ...
        //$where_zuhe = $children ? " AND (" . $children.' or goods_id IN(SELECT goods_id FROM ' . GOODSCAT . " AS g WHERE {$children}" . ')) ' : '';
	   ...
	   // $where .= ' '.get_powerful(GOODS);   //获取权限
		...
		//echo $where;
        /* 记录总数 */
       // $sql = "SELECT COUNT(*) FROM " .GOODS. " AS g WHERE is_delete='$is_delete' $where";
        $filter['record_count'] = $GLOBALS['db']->count_info(GOODS.' AS g ','*'," is_delete='$is_delete' $where $where_zuhe");

		if($where_cat){
           $filter['zhucat_count'] = $GLOBALS['db']->count_info(GOODS.' AS g ','*'," is_delete='$is_delete' $where $where_cat");
        ...
        $sql = "SELECT goods_id,gifts_id, goods_title, goods_thumb,click_count,sale_count,goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot,point_rate, sort_order, goods_number, add_time,add_user,last_update,update_user , " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . GOODS . " AS g WHERE is_delete='$is_delete' $where $where_zuhe " .
	...
	foreach($rss as $k => $row){
    	...
    	if(!empty($gifts[$row['gifts_id']])){
    		$row['need_money'] = $gifts[$row['gifts_id']]['need_money'];
    	}else $row['need_money']='';
...
function get_review($wheresql)
{
    /* 过滤条件 */
      ...
      //  {
       ...
       // }
        ...
        /* 记录总数 */
       // $sql = "SELECT COUNT(*) FROM " .GOODS. " AS g WHERE is_delete='$is_delete' $where";
       // $filter['record_count'] = $GLOBALS['db']->count_info(REVIEW.' AS R ','*',"  $where ");
    ...
    $rss = $GLOBALS['db']->arrQuery($sql);
	foreach ($rss as $k=>$v){
		$rss[$k]['adddate']=local_date('Y-m-d h:m:s',$rss[$k]['adddate']);
...
function get_review_list($page=1,$page_size=20){
	...
	foreach ($review_list as $k=>$v){
		$review_list[$k]['adddate']=local_date('Y-m-d h:m:s',$review_list[$k]['adddate']);
...
function get_reviews($goods_id,$rid){
	...
	foreach ($review_list as $k=>$v){
		...
		//echo "select email from ".USERS ." where user_id=".$v['user_id'];
		...
		foreach ($reply as $k1=>$v1){
			$reply[$k1]['adddate'] = local_date('M-d/Y h:m:s',$reply[$k1]['adddate']);
...
function get_one_inquiry($goods_id,$iid){
	...
	//$inquir['review_rate'] = round($review_stat['review_rate'],2);
	...
	foreach ($inquiry_list as $k=>$v){
		...
		//echo "select email from ".USERS ." where user_id=".$v['user_id'];
		if(!empty($inquiry_list[$k]['adddate']))$inquiry_list[$k]['adddate']=local_date('Y-m-d h:m:s',$inquiry_list[$k]['adddate']);
...
function get_inquiry($wheresql)
{
  ...
  //  {
    ...
    //    $filter['real_goods']       = $real_goods;
       ...
       // $children = $filter['cat_id'] > 0 ? get_children($filter['cat_id']):'';
      //  $where_zuhe = $children ? " AND (" . $children." or " . get_extension_goods($children) . ") " : '';
       ...
       // $sql = "SELECT COUNT(*) FROM " .GOODS. " AS g WHERE is_delete='$is_delete' $where";
       // $filter['record_count'] = $GLOBALS['db']->count_info(REVIEW.' AS R ','*',"  $where ");
		$filter['record_count'] = $GLOBALS['db']->getOne("select count(*) from ".PRO_INQUIRY." i,".GOODS." g where ".$where.$wheresql);
        /* 分页大小 */
       // exit();
        $filter = page_and_size($filter);
