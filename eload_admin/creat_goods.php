...
<?  flush();
    ...
    $pagesize = !empty($_POST['pagesize']) ? intval($_POST['pagesize']) : 50;
    ...
    if ($cat_id[0] != 0){
		foreach ($cat_id as $vv){
			$cat_temp_Arr[$vv] = $catArr[$vv];
		}
		$catArr = $cat_temp_Arr;
	}
	
	$total = count($catArr)*2; //总共需要操作的记录数
	$pix = $width / $total; //每条记录的操作所占的进度条单位长度
	$progress = 0;  //当前进度条长度
	
	creat_goods_list($catArr,$listperArr,$list_odr_Arr);
	creat_goods_list($catArr,$gridperArr,$grid_odr_Arr,'g');
	?>
	<script language="JavaScript">
			...
			foreach ($listperArr as $val){
				...
				for ($p=1;$p<=$max_page;$p++){
					...
					foreach ($list_odr_Arr as $key => $odr){
						$file_name = $url_title.'-'.$k.'-'.$key.'-'.$val.'-'.$p.'.htm';
						$re_url  = $url .$file_name.'?m=category&id='.$k.'&page='.$p.'&page_size='.$val.'&odr='.$key.'&display='.$layout.'&creat=1';
						...
						foreach ($listperArr as $vl){
							if ($val == $vl) $style = 'red';
							//href='/htm/$url_title/$url_title-$k-$key-$vl-1.htm'
						...
						?>
						<script language="JavaScript">
                         updateProgress(" 正在更 新“<?php echo $v['cat_name'].' '.$layout. ' pages';?>” ... ", <?php echo min($width, intval($progress)); ?>);
                        </script>
						<?
					}
				}
