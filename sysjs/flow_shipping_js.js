$(document).ready(function(){
	...
	$(".shipping_method").live('click', function(){
		...
		freeyunfei  = parseFloat($("#freesm"+shipping_id).attr("orgp"));
	 ...
	 function clk_tracking_number(){
		...
		thisprice = parseFloat($('#Need_Traking_number_fee'+shipid).attr("orgp"));
		freesmprice = parseFloat($('#freesm'+shipid).attr("orgp"));
        var el = $("#shipping_sub_total"), free_sub_total = el.attr("orgp");
	...
	$(".baofei").live('click', function(){
		yunfei  = $("#baofei").attr("orgp");
		
		if($(this).attr("checked") == true){ 
			$("#insurance").html(yunfei);
			$("#insurance").attr("orgp",yunfei);
		}else{
			$("#insurance").html(0);
	...
	function jisuan_total(){
		...
		sub_total = $("#shipping_sub_total").attr("orgp");
        ...
        cheknum = 0;
		insurance    = parseFloat($("#insurance").attr("orgp"));//保险费
		xx_sub_total = parseFloat($("#items_sub_total").attr("orgp"));//单价
        ...
        try {show_my_shop_price('');} catch(e) {}; 
	}
