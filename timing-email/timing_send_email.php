...
$result_msg    = '';
echo  '<pre>';
$sql = "SELECT u.firstname,u.lastname,u.email,u.user_id,u.lang,o.order_id,FROM_UNIXTIME(o.add_time,'%Y-%m-%d') as date,o.order_sn,DATEDIFF(curdate(),FROM_UNIXTIME(o.add_time,'%Y-%m-%d')) as days FROM eload_users AS u, eload_order_info AS o WHERE u.user_id = o.user_id AND DATEDIFF(curdate(),FROM_UNIXTIME(o.add_time,'%Y-%m-%d')) = 7 and o.order_status = 0 and u.is_unsub = 0 ";
		...
		if ($alrady_email == 0){
			...
			/* 订单商品 */
					...
					if(empty($lang) || $lang == 'en') {
						$mail_subject = $mail_conf['en'][20];
						$mail_body    = $Tpl->fetch(ROOT_PATH.'eload_admin/email_temp/en/20.html');															
					} elseif (!empty($lang)){
						$mail_subject = $mail_conf[$lang][20];
						$mail_body    = $Tpl->fetch(ROOT_PATH.'eload_admin/email_temp/'. $lang .'/20.html');																					
					}
