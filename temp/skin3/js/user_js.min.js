...
$(document).ready(function(){
	...
	$("#signinform").validate({
		...
		submitHandler: function() {
			...
			var password = encodeURIComponent($('#passwordsign').val());
			var flow = $('#flow').val();
			var ref = $('#ref').val();
			$.ajax({
				   type: "POST",
				   ...
				   dataType:"text",
					beforeSend:function(){$("#ajaxmsg").css("color","#333").show().html("<img src='http://cloud4.faout.com/imagecache/A/images/styleimg/indicator.gif' id='verify' > Signing ...");	},
					success: function(msg){
						$('#passwordsign').val('');
						$("#ajaxmsg").css("color","#c00").html(msg);
						if (msg.indexOf('Success')>=0){
							var msg = js_success_msg;
							var _time = 2;
							$("#ajaxmsg").css("color","#060"); 
							$("#ajaxmsg").html(msg);
							var setIntervalFun =  setInterval(function(){
								if(_time>0){
                                    ...
                                    location.href ="http://"+ window.location.host +'/daily-deals/';
                                   else
									location.href = DOMAIN_USER + "/" + js_cur_lang_url + "m-users.htm";
							           }
	...
	$("#register").validate({
		...
		submitHandler: function() {
			...
			if($("#newsletter_register").attr("checked"))
			{
			...
			{
				newsletter_register = 0;
			}			
			$('#subCreate').attr("disabled",true);
			$.ajax({
				type: "POST",
				url: '/'+js_cur_lang_url + "m-users-a-a_join.htm",
				data: "email1="+email+"&password="+password+"&verifcode="+verifcode+"&newsletter_register="+newsletter_register,
				dataType:"text",
				beforeSend:function(){
					$("#ajaxmsg1").show().html("<img src='http://cloud4.faout.com/imagecache/A/images/styleimg/indicator.gif' id='verify' Creating ...>");	
                 }, 
				success: function(msg){
					$("#ajaxmsg1").html(msg);
					if (msg.indexOf('ok')>=0){
...
//邮箱修改
 $("#form-edit_email").validate({
            ...
            submitHandler: function() {
                ...
                $.post('/'+ js_cur_lang_url +'m-users-a-edit_email.html', {
                    email: _email,
                    ...
                    try {
                        ...
                        if (data.success) {
                            var method = data.confirm_code ? '' : 'succeedInfo', msg = ''+ edit_successfully +'';
                        }
                        else {
                        ...
                        log(e);
                        var method = 'errorInfo', msg = ''+ edit_failed +'';
                    }
	...
	$(".theAddressForm").each(function(){
		 $(this).validate({
			...
			messages: {
				firstname: {required: firstname_msg,maxlength: firstname_maxlength_msg,checkUserName:not_number,minlength:enter_least},
				lastname: {required: lastname_msg,maxlength: lastname_maxlength_msg,checkUserName:not_number,minlength:enter_least},
				...
				addressline2: {maxlength: addressline2_maxlength_msg},
	...
	$(".theBillAddressForm").each(function(){ 
		 $(this).validate({
			...
			messages: {
				firstname: {required: bill_firstname_msg,maxlength: bill_firstname_maxlength_msg,checkUserName:not_number,minlength:enter_least},
				lastname: {required: bill_lastname_msg,maxlength: bill_lastname_maxlength_msg,checkUserName:not_number,minlength:enter_least},
				...
				country: {required:bill_country_msg},
   ...
   // 
	...
	//更改订单状态
    $("#ChangeOrderState").live("click",function(){
 ...
 //   });
    ...
    $('.a-delete_order').click(function() {
        if (confirm(user_js_func_deleteOrder_0)) {
            ...
            $.post('/'+ js_cur_lang_url +'m-users-a-delete_order.html', (pid == undefined ? 'bid' : 'id') + '=' + id + '&pid=' + pid, function(data) {
...
$(function(){
	...
	//取消删除订单
	$(".cancelOrder_btns .no").live("click",function(){
		layer.close(delOrderPopIndex);
	});
	//确认删除
	$(".cancelOrder_btns .yes").live("click",function(){
	...
	//选择原因
	$("#delOrderReason").live("change",function(){
		$(".cancleOrderMsg").html('');
	});
...
function delOrderPop(url){
	var html ='<div id="cancelOrder" ><h4  class="fb f_Arial" style="height:34px">'+jsLg.myAccount.deteTips_2+'</h4><select id="delOrderReason"><option  value="0">'+jsLg.myAccount.deteTips_1+'</option><option value="1">'+jsLg.myAccount.deteTips_3+'</option><option value="2">'+jsLg.myAccount.deteTips_4+'</option><option value="3">'+jsLg.myAccount.deteTips_5+'</option><option value="4">'+jsLg.myAccount.deteTips_6+'</option><option value="5">'+jsLg.myAccount.deteTips_7+'</option><option value="6">'+jsLg.myAccount.deteTips_8+'</option></select><p class="cancleOrderMsg"></p><div class="cancelOrder_btns"><a href="javascript:void(0);" class="allBtn yes" data-url="'+url+'">' + jsLg.yes + '</a><a href="javascript:void(0);" class="hBtn no">'+ jsLg.no +'</a></div></div>';
	delOrderPopIndex = $.layer({
		type: 1,
		title: jsLg.myAccount.deteTips_9,
		offset : [($(window).height()-180)/2 - 20 + 'px', ''],
		area: ['380px','180px'],
		page: {
			html: html
		}
