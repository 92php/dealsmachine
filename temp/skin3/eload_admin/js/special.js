...
SPECIAL.specialGoods = {
    ...
    list: function(){
        ...
        $('.a-delete').click(function(){
            confirm('确认要删除该商品?') && me['delete'](this.id.substr(2));
        });
        /*
        $('#btnSubmit').click(function() {//删除按钮
            var values = $('input[name=checkboxes]:checked').map(function() {
                return $(this).val();
            }).get().join(',');
            values && confirm('确认要删除选中商品?') && me['delete'](values);
        });
		*/
    },
    
    'delete': function(id) {//删除产品 by mashanling on 2012-11-03 09:50:36
        if (!id) {
            return;
        }
        
        $.post(SPECIAL.url + '?act=delete_special_goods', 'auto_id=' + id, function(data){
            C.callback(data, location.href, L.del + L.success);
        });
    },
    ...
    loadPositions: function() {
        var id = selectSpecialId.val(), html = '<option value="">请选择</option>', tpl = '<option value="{0}"{1}>{2}</option>';
